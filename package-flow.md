...
**关键细节**：当返回 404 时，代码会额外调用 `self.latest_version()` 来**尝试**区分是"包不存在"还是"版本不存在"：
- 索引查询成功且能找到该包的其他版本 → `PackageError::VersionNotFound(spec, latest_version)`
- 其他所有失败情况 → 退回到 `PackageError::NotFound(spec)`

⚠️ **重要：`latest_version()` 失败 ≠ 包不存在**。`Err(_)` 分支可能由以下多种原因触发，全部被错误地归为 `NotFound`：

| `latest_version()` 失败原因 | 是否真的表示包不存在 | 实际错误语义 |
|---------------------------|---------------------|------------|
| 索引下载成功但包名不在索引中 | ✅ 是 | 包确实不存在 |
| 索引下载失败（网络超时/DNS 错误等） | ❌ 否 | 网络故障，无法确认 |
| 索引 JSON 解析失败 | ❌ 否 | 数据损坏/格式不兼容 |
| 索引返回 404 | ❌ 否 | 索引服务异常 |

这意味着下载失败（404）时会**额外触发一次索引查询**，但查询失败时会丢失真实原因，统一表现为 `NotFound`。这是当前实现的一个已知局限：无法区分"包真的不存在"和"因网络/解析问题无法确认包是否存在"。
...
**来源 B（`universe.package()` 内部 404 分支）**："远程返回 404，且无法确认是包不存在还是版本不存在"
- 触发条件：HTTP 404 且 `latest_version()` 返回 `Err`
- ⚠️ **注意：这是一个语义近似**。`Err(_)` 可能是"包真的不存在"，也可能是"索引下载失败/解析失败导致无法确认"
...
| HTTP 404 + 索引查询成功 + 包存在 | 可查到最新版本号 | `VersionNotFound(spec, latest)` 沿 `?` 透传 | — |
| HTTP 404 + 索引查询成功 + 包不存在 | 返回"找不到包"错误 | `NotFound` 沿 `?` 透传返回 | 来源 B（准确） |
| HTTP 404 + 索引查询失败（网络/解析问题） | 返回网络/解析错误 | `NotFound` 沿 `?` 透传返回（**语义失真**） | 来源 B（不准确） |
...
- **错误分层，沿 `?` 透传**：下载分支内 `universe.package()` 的错误通过 `?` 直接向上传播，不会被吞掉
  - 网络不可用/超时 → `NetworkFailed`
  - 404 + 索引查询成功 + 包存在 → `VersionNotFound`（带最新版本号提示）
  - 404 + 索引查询失败/包不存在 → `NotFound`（来源 B，语义近似，可能失真）
- **`NotFound` 的双重语义（都不精确）**：
  - **来源 A**（第 123 行函数末尾）："没有可用来源能找到这个包"——本地未找到 + 下载分支被跳过或未返回
  - **来源 B**（`universe.package()` 内部 404 分支）："远程返回 404 且索引查询失败"——通过 `?` 透传。⚠️ 这不等价于"包不存在"，可能是网络/解析问题导致无法确认，属于语义近似
